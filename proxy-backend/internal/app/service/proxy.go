...
func NewProxyService(notFoundService *NotFoundPageService, log zerolog.Logger) *ProxyService {
	return &ProxyService{notFoundService, log}
}
	...
	rp.ErrorHandler = func(w http.ResponseWriter, r *http.Request, err error) {
		...
		if proxyErr := r.Header.Get("X-Proxy-Error"); proxyErr != "" {
			if strings.HasSuffix(err.Error(), "no such host") {
				s.notFoundService.Render404(c, r.URL.Path, r.URL.Host, "no such host", username)
				return
			}
			if strings.HasSuffix(err.Error(), "connection refused") {
				s.notFoundService.Render404(c, r.URL.Path, r.URL.Host, "connection refused", username)
				return
			}
